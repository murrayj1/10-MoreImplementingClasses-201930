    ...
    *** See the PDFs, especially dimensions.pdf, to help you understand this.
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        *** See   dimensions.pdf   to understand the following! ***
        ...
        # ---------------------------------------------------------------------
        a = intersection_center.x
        b = intersection_center.y
        HorRec_TopLeft = rg.Point(a - width/2, b - letter_thickness/2)
        HorRec_BotRight = rg.Point(a + width/2, b + letter_thickness/2)
        VerRec_TopLeft = rg.Point(a - letter_thickness/2, b - letter_thickness/2)
        VerRec_BotRight = rg.Point(a + letter_thickness/2, b -letter_thickness/2 + height)
        self.h_rect = rg.Rectangle(HorRec_TopLeft, HorRec_BotRight)
        self.v_rect = rg.Rectangle(VerRec_TopLeft, VerRec_BotRight)
    ...
    def clone(self):
        ...
        #######################################################################
        nh_rect = rg.Rectangle(self.h_rect.corner_1, self.h_rect.corner_2)
        nv_rect = rg.Rectangle(self.v_rect.corner_1, self.v_rect.corner_2)
        nh_rect.fill_color = self.h_rect.fill_color
        nh_rect.outline_color = self.h_rect.outline_color
        nv_rect.outline_color = self.v_rect.outline_color
        nv_rect.fill_color = self.v_rect.fill_color
        self.v_rect = nv_rect
        self.h_rect = nh_rect
        return 
